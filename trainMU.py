...
import numpy as np
from MODELmobileunet import get_model
...
scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=50, gamma=0.5)  # 每50个epoch将学习率降低为原来的0.5倍
...
def train():
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            model_path = f'MobileUnet_epoch_{epoch + 1}.pth'
            torch.save(model.state_dict(), model_path)
            print(f'Model saved as {model_path}')

    torch.save(model.state_dict(), 'MobileUnet.pth')
    print('Model saved as MobileUnet.pth')
