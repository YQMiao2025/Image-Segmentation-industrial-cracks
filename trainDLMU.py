...
import numpy as np
from MODELdl3plusmunet import get_model
...
scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=30, gamma=0.7)  # 每30个epoch将学习率降低为原来的0.7倍
...
def train():
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            model_path = f'DLMU_epoch_{epoch + 1}.pth'
            torch.save(model.state_dict(), model_path)
            print(f'Model saved as {model_path}')

    torch.save(model.state_dict(), '.pth')
    print('Model saved as DLMU.pth')
